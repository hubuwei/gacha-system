...
server {
    ...
    # 认证服务 - 使用rewrite保持完整路径
    location ~ ^/api/auth/(.*)$ {
        proxy_pass http://auth_service/api/auth/$1;
...
docker cp /tmp/fix-nginx.sh gacha-frontend:/tmp/fix-nginx.sh
docker exec gacha-frontend bash /tmp/fix-nginx.sh

echo "测试搜索接口..."
sleep 2
curl -s http://localhost/api/auth/search?keyword=test001 | head -c 200
